module save_the_flag::OtterLoan {
    ...
    fun test_flashloan() {
        let lender = @0x111;
        let borrower = @0x222;
        let _contract = @save_the_flag;

        let scenario_val = test_scenario::begin(lender);
        ...
        {
            let flash_lender = test_scenario::take_shared<FlashLender<SUI>>(scenario);
            let (tokens, receipt) = loan(&mut flash_lender, 100, test_scenario::ctx(scenario));
